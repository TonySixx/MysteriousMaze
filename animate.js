...
export function createProtectiveShield() {
  playSound(spell2SoundBuffer, 0.8);
  if (protectiveShield) {
    scene.remove(protectiveShield);
  }
  ...
  const material = new THREE.ShaderMaterial({
    uniforms: {
      time: { value: 0 },
      color: { value: new THREE.Color(0x6f03fc) }
    },
    vertexShader: `
  ...
  `,
    fragmentShader: `
      ...
      void main() {
          ...
          float opacity = 0.05 + pulse * 0.1;
          gl_FragColor = vec4(color, opacity);
      }
  `,
    transparent: true,
    blending: THREE.AdditiveBlending, // Přidání míchání barev
    depthTest: true, // Povolení hloubkového testu
    depthWrite: false, // Zakázání zápisu do hloubky
    side: THREE.DoubleSide
  });

  protectiveShield = new THREE.Mesh(geometry, material);
  protectiveShield.scale.setScalar(1.2);
  protectiveShield.position.y += 2; // Posuneme štít výš o 1 jednotku
  player.add(protectiveShield);

  protectiveShield.userData = {
    startTime: Date.now(),
    duration: 2000 // 2 sekundy
  };
}

export function updateProtectiveShield(deltaTime) {
  if (protectiveShield) {
    const elapsedTime = Date.now() - protectiveShield.userData.startTime;
    if (elapsedTime < protectiveShield.userData.duration) {
      protectiveShield.material.uniforms.time.value = elapsedTime / 1000;
    } else {
      player.remove(protectiveShield);
      protectiveShield = null;
    }
  }
}

// Přidejte tuto funkci pro blokování poškození
export function isProtectiveShieldActive() {
  return protectiveShield !== null;
}
